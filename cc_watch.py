#!/usr/bin/env python3
"""
cc-watch — Claude Code cache TTL inspector

Usage:
  cc-watch              List 5 most recent sessions
  cc-watch --n 10       List N most recent sessions
  cc-watch --id <id>    Inspect a specific session (partial UUID OK)
"""
...
def print_detail(d: dict) -> None:
    ...
    print()
    print(TL + H * (W + 2) + TR)
    print(box_line(b("Claude Code Watch Inspector")))
...
def print_list_header(n: int) -> None:
    ...
    print()
    print(f"  {b('Claude Code Watch')}  {dim('·')}  {dim(f'{n} most recent sessions')}  "
          f"{dim('·')}  {dim(now_str)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  cc-watch              # list 5 most recent sessions\n"
            "  cc-watch --n 10       # list 10 most recent\n"
            "  cc-watch --id d4b247b7  # inspect a session (partial UUID OK)\n"
        ),
    )
