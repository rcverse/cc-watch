...
cc-cache v2 is a lightweight macOS terminal tool for inspecting Claude Code session cache state and managing per-session Reminder and KeepAlive workflows.
...
```text
cc-cache                     # List View
cc-cache --n N               # List View with N recent sessions
cc-cache --id <partial-id>   # Session Workspace
cc-cache --json              # Stable JSON API
cc-cache --json --id <id>    # Stable JSON API for one session
...
`--watch` is not a public command. Live refresh is internal TUI behavior.
