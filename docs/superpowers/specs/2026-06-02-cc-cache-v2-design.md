...
cc-cache v2 is a terminal TUI for inspecting Claude Code session cache health and managing bounded keep-alive/reminder workflows.
...
`--watch` is not part of v2. Live refresh is internal behavior, not a mode, and must not reintroduce watch intervals or polling semantics.
...
Reminder notifications fire only while cc-cache is running.
...
- Use panel boundaries for major regions: List, Session Evidence, Controls, Config fields, Validation, and Help.
...
- Keep dangerous KeepAlive states pinned inside the visible KeepAlive card in Controls. A help overlay must not hide an active countdown, sending, confirming, or failure state.
...
- Normal Workspace footer: `r remind   k keepalive   c copy id   b/esc back   ? help   q quit`.
- Countdown footer: `s send now   x cancel instance   b back   ? help   q quit`.
- Confirming footer: `x stop waiting   b back   ? help   q quit`.
...
| Copy/show full session ID | `enter` on focused action | `c` |
| Manual refresh for this session | `enter` on focused action | none |
| Help | `enter` on focused action | `?` |
| Back to list | `enter` on focused action | `b` / `esc` |
| Quit | `enter` on focused action | `q` |

`c` is reserved for copy ID as both a shortcut and visible action. KeepAlive cancellation uses `x` to avoid the earlier context-sensitive `c` conflict. If clipboard integration is unavailable, `c` must show the full session ID in a selectable/copyable status line rather than failing silently.
...
enter activate focused action   arrows move   ? help
```

### 10.6 Shortcut Rules

- `?` opens help everywhere.
...
- `s` means save in Config Editor and send-now only inside the KeepAlive area/state. The visible help overlay must reflect the current meaning.
- `x` means cancel/dismiss current KeepAlive instance only where such an instance exists.
- Disabled or unavailable actions remain visible in help with a short reason when they are safety-relevant.
...
+-------------------------------------------------------------+
| cursor move  enter open focused  shortcuts: r remind  k keepalive  ? help  q quit |
+-------------------------------------------------------------+
```
...
|                                                                       |
| cc-cache looks for JSONL files under:                                  |
|   ~/.claude/projects/                                                 |
|                                                                       |
...
cc-cache cannot discover sessions until Claude Code creates that directory.
```
...
```text
+ cc-cache -------------------------------- ambiguous session id: d4b +
...
```text
+ cc-cache / workspace-api / d4b247b7 ----------------------------------------+
| Session details                                                              |
|                                                                              |
| Status                                                                       |
...
|   [ ] KeepAlive  [x] auto-send · trigger 5m · scope 1 send   Off. No message.|
+------------------------------------------------------------------------------+
| arrows move focus   enter/space act   r remind   k keepalive   c copy id   ? |
+------------------------------------------------------------------------------+
```
...
+------------------------------------------------------------------------------+
| arrows choose action   enter act   s send now   x cancel instance   ? help    |
```
...
|     Scope    0 / 1 sends                                                     |
+------------------------------------------------------------------------------+
| arrows choose action   enter act   s send now   x dismiss   ? help            |
```
...
+------------------------------------------------------------------------------+
| arrows choose action   enter act   x stop waiting   b back   ? help           |
```
...
#### 10.7.12 Help Overlay

The help overlay is a temporary command reference. It should not hide active dangerous state lines, and it must explain both cursor navigation and shortcuts.
...
|   space           toggle focused checkbox-style control                      |
|   ?               toggle help                                                |
|   q               quit                                                       |
|                                                                              |
| List View                                                                    |
...
|                                                                              |
| Session Workspace                                                            |
|   k      toggle KeepAlive monitoring                                         |
|   r      toggle Reminder                                                     |
|   s      send KeepAlive now, only during countdown/manual-ready               |
|   x      cancel/dismiss current KeepAlive instance                            |
|   c      copy or show full session ID                                         |
|   b/esc  back to list                                                        |
|                                                                              |
| Config Editor                                                                |
|   up/down next field                                                         |
|   s      save valid config                                                   |
|   d      reset defaults, requires confirmation                               |
+------------------------------------------------------------------------------+
```
...
Notifications are always attempted for reminder and KeepAlive events while cc-cache is running, but notification failures must not create noisy retry loops.
...
Runtime state is in-memory and is discarded when cc-cache exits.
