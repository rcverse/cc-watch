# cc-cache v2 Meta Implementation Plan

> Active phase checklists live beside this file as `phase-XX-*.md`. Use `docs/superpowers/runbooks/2026-06-03-cc-cache-v2-implementation-runbook.md` as the execution protocol and `docs/superpowers/progress/cc-cache-v2-progress.md` as the progress ledger.
...
**Goal:** Build cc-cache v2 as a Go/Bubbletea single-binary terminal app that preserves v1 parser behavior, adds safe live TUI workflows for Reminder and bounded KeepAlive, provides stable JSON output, and first ships as a locally installable binary.
...
- Current tracked runtime is a single executable Python script at `cc_cache.py`.
- Current tracked installer is `install.sh`, which force-symlinks `$HOME/.local/bin/cc-cache` to `/Users/richardchen/Dev/cc-cache/cc_cache.py`.
...
- Preserve the existing usable command path during migration by leaving root `cc_cache.py` in place until the Go binary has passed acceptance verification and the user explicitly approves the local install/symlink switch.
- Archive v1 by copying it into `archive/v1/` during implementation, not by moving the root script early. Moving root `cc_cache.py` before switchover would break the currently installed symlink.
...
- `archive/v1/cc_cache.py` - copied v1 script for provenance.
...
- `cmd/cc-cache/main.go` - CLI entry point only.
...
- `internal/session/testdata/smoke-home/.claude/projects/-tmp-cc-cache/11111111-1111-1111-1111-111111111111.jsonl` - deterministic safe smoke session.
- `internal/session/testdata/smoke-home/.config/cc-cache/config.json` - deterministic smoke config with KeepAlive auto-send disabled.
- `internal/config/model.go` - config model and defaults.
- `internal/config/store.go` - config file load/save/reset behavior for `~/.config/cc-cache/config.json`.
...
|---|---|---|---|---|
| v1 migration | Existing symlink breaks if root `cc_cache.py` moves early | User command fails | Copy archive first; leave root script until verified switchover | Phase 1 command-path checks |
...
- Approve local command-path switchover separately before replacing `$HOME/.local/bin/cc-cache`.
