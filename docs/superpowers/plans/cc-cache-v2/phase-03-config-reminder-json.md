# cc-cache v2 Phase 3: Config Reminder Json
...
- [x] **Step 3.1: Write failing config tests**
  ...
  - Assertions: default config exactly matches PRD/design values, config loads from `~/.config/cc-cache/config.json`, invalid JSON falls back with visible warning state, save writes only valid config, reset restores defaults, cancel path performs no write, active per-session state is not mutated by config changes.
...
- [x] **Step 3.6: Implement JSON output**
  ...
  - Behavior: `cc-cache --json` and `cc-cache --json --id <id>` print JSON and exit without starting Bubbletea, watchers, notifications, or KeepAlive subprocesses.
