# cc-watch v2 Phase 11: Cli Integration
...
- [x] **Step 11.1: CLI command tests**
  ...
  - Assertions: `cc-watch`, `--n N`, `--id <partial-id>`, `--json`, `--json --id <id>`, `--remind`, `config`, `--help`, `--version`, and retired watch flag rejection.
  - Run: `go test ./internal/app ./...`.
  - Expected: pass.

- [x] **Step 11.2: JSON command smoke**
  - Run: `HOME="$PWD/internal/session/testdata/smoke-home" go run ./cmd/cc-watch --json`.
  - Expected: valid JSON to stdout and process exits without launching TUI.
  - Run: `HOME="$PWD/internal/session/testdata/smoke-home" go run ./cmd/cc-watch --json --id 11111111`.
  - Expected: selected-session JSON matching `schema_version: 1` and no `error`.
  - Run: `HOME="$PWD/internal/session/testdata/smoke-home" go run ./cmd/cc-watch --json --id no-such-session`.
