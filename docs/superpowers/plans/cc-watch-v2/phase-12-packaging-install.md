# cc-watch v2 Phase 12: Local macOS Install Implementation Plan
...
**Goal:** Provide a verified simple local macOS install path for the Go binary without touching the real `$HOME/.local/bin/cc-watch` during implementation.

**Architecture:** `install.sh` becomes the local installer for the verified Go binary. It builds the binary, verifies it, and installs a copied executable into `$HOME/.local/bin/cc-watch` only when invoked with explicit approval (`--yes`). Tests exercise the installer with a temporary `HOME`, preserving the current live command path and v1 rollback files.
...
- Build a local Go binary artifact at `dist/cc-watch`.
...
- Keep root `cc_watch.py` and `archive/v1/*` intact for rollback/reference.
- Update docs/progress to say public release packaging remains deferred.

Out of scope:

- Running `install.sh` against the real user `$HOME`.
- Replacing the live `$HOME/.local/bin/cc-watch` command path.
...
- Modify: `install.sh`
- Modify: `README.md`
- Modify: `docs/superpowers/plans/cc-watch-v2/phase-12-packaging-install.md`
...
```bash
git check-ignore internal/session/testdata/smoke-home/.claude/projects/-tmp-cc-watch/11111111-1111-1111-1111-111111111111.jsonl
```
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go build -o dist/cc-watch ./cmd/cc-watch
```

Expected: exits 0 and creates `dist/cc-watch`. The file is ignored by git.
...
```bash
dist/cc-watch --version
dist/cc-watch --help
HOME="$PWD/internal/session/testdata/smoke-home" dist/cc-watch --json
```
...
HOME="$TEST_HOME" CC_WATCH_BUILD_DIR="$BUILD_DIR" "$ROOT/install.sh" --yes
...
Expected before implementation: fail because current `install.sh` installs a symlink to `cc_watch.py`, which does not support `--version`.
...
Expected: pass. This uses a temporary `HOME` and does not touch the real `$HOME/.local/bin/cc-watch`.
...
```bash
TMP_HOME="$(mktemp -d /private/tmp/cc-watch-install-dry.XXXXXX)"
HOME="$TMP_HOME" ./install.sh --dry-run
test ! -e "$TMP_HOME/.local/bin/cc-watch"
rm -rf "$TMP_HOME"
```
...
```bash
TMP_HOME="$(mktemp -d /private/tmp/cc-watch-install-refuse.XXXXXX)"
set +e
...
The script builds the Go binary and installs a copied executable to `$HOME/.local/bin/cc-watch`.
...
Modify `docs/superpowers/progress/cc-watch-v2-progress.md` current state to:

```markdown
- Current phase: Phase 12 - Local macOS Install
- Current phase file: `docs/superpowers/plans/cc-watch-v2/phase-12-packaging-install.md`
...
```bash
rg -n 'Homebrew|goreleaser|GitHub Releases|public release|deferred|out of scope' README.md docs/superpowers/specs/2026-06-18-cc-watch-v2-product-reality.md docs/superpowers/plans/cc-watch-v2/PLAN.md
```
...
```bash
command -v cc-watch || true
ls -l "$HOME/.local/bin/cc-watch" || true
```
...
- no real `$HOME/.local/bin/cc-watch` replacement during implementation;
- installer does not overwrite `cc_watch.py` through an existing symlink;
...
- If the user explicitly approves replacing `$HOME/.local/bin/cc-watch`, perform it as a separate operator action after this phase, with before/after `command -v`, `ls -l`, `cc-watch --version`, and JSON smoke checks.
