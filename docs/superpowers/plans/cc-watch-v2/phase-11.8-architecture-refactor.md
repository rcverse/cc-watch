# cc-watch v2 Phase 11.8 Architecture Refactor Implementation Plan
...
- `cc-watch` opens the TUI list using the latest sessions.
- `cc-watch --id <partial>` opens one selected workspace or ambiguous selection state.
- `cc-watch --json` emits the accepted schema from `docs/adr/2026-06-03-json-output-schema.md`.
- `cc-watch config` opens the config editor without session discovery.
- TUI refresh keeps list/session data current without exposing a public watch command or flag.
...
- `CONTEXT.md` - small cc-watch domain glossary used by future architecture work. Keep it under 80 lines.
...
- `docs/superpowers/plans/cc-watch-v2/PLAN.md` - add Phase 11.8 to phase index.
- `docs/superpowers/progress/cc-watch-v2-progress.md` - mark current phase as Phase 11.8 after implementation starts and update verification ledger before stopping work.
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go test ./internal/snapshot
```

Expected: fail with `package github.com/richardchen/cc-watch/internal/snapshot is not in std` or undefined `Build`, `ConfigOnly`, and snapshot types.
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go test ./internal/session ./internal/config ./internal/jsonout
```
...
- Import `github.com/richardchen/cc-watch/internal/snapshot`.
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go test ./internal/app ./internal/jsonout
```
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go test ./internal/tui ./internal/app
```
...
In `docs/superpowers/plans/cc-watch-v2/PLAN.md`, add Phase 11.8 between Phase 11.7 and Phase 12:
...
In `docs/superpowers/progress/cc-watch-v2-progress.md`, set:

```markdown
- Current phase: Phase 11.8 - Architecture Refactor
- Current phase file: `docs/superpowers/plans/cc-watch-v2/phase-11.8-architecture-refactor.md`
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go test ./internal/snapshot ./internal/app ./internal/jsonout ./internal/tui
```
...
```bash
GOCACHE=/private/tmp/cc-watch-go-build GOMODCACHE=/private/tmp/cc-watch-go-mod go build -o /private/tmp/cc-watch-phase118/cc-watch ./cmd/cc-watch
```

Expected: command exits 0 and writes `/private/tmp/cc-watch-phase118/cc-watch`.
...
```bash
HOME="$PWD/internal/session/testdata/smoke-home" /private/tmp/cc-watch-phase118/cc-watch --json
```
...
Expected: only intended source/doc changes. No `$HOME/.local/bin/cc-watch` replacement, no release artifacts in repo, no real Claude send.
